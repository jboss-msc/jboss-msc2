...
public class ServiceTestCase extends AbstractServiceTest {
    ...
    public void installAndRemoveService() {
        ...
        try {
            ServiceBuilder<Void> serviceBuilder = txnController.newServiceContext(txn1).addService(serviceRegistry, firstSN);
    ...
    public void installAndRemoveServiceDependent() {
        ...
        try {
            final ServiceBuilder<Void> secondServiceBuilder = txnController.newServiceContext(txn1).addService(serviceRegistry, secondSN);
            ...
            final ServiceBuilder<Void> firstServiceBuilder = txnController.newServiceContext(txn1).addService(serviceRegistry, firstSN);
    ...
    public void installAndRemoveServiceDependentMultipleTxns() {
        ...
        final ServiceController firstServiceController;
        try {
            final ServiceBuilder<Void> firstServiceBuilder = txnController.newServiceContext(txn2).addService(serviceRegistry, firstSN);
            firstService = new TestService(firstSN, firstServiceBuilder, false, new DependencyInfo<Void>(secondSN));
            firstServiceBuilder.setService(firstService);
            firstServiceController = firstServiceBuilder.install();
        } finally {
